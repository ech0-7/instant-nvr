...
def get_rays_coord(H, W, K, R, T):
    ...
    i, j = np.meshgrid(np.arange(W, dtype=np.float32),
                       np.arange(H, dtype=np.float32),
                       indexing='xy')
    coord = np.stack([i, j], axis=-1).reshape(-1, 2).astype(np.uint8)

    # i = i + 0.5  # align pixel center, might not be useful in monocular case
    # j = j + 0.5  # align pixel center, might not be useful in monocular case
    xy1 = np.stack([i, j, np.ones_like(i)], axis=2)
    pixel_camera = np.dot(xy1, np.linalg.inv(K).T)
    pixel_world = np.dot(pixel_camera - T.ravel(), R)
    # calculate the ray direction
...
def sample_ray_h36m_mse(img, msk, error_msk, K, R, T, bounds, nrays, split):
    H, W = img.shape[:2]
    ray_o, ray_d = get_rays(H, W, K, R, T)
    ...
    # 要不要这样做? 这样就没法采集到边缘部分了, 而这一部分对涨点很有用?
    error_msk[msk == 100] = 0

    nsampled_rays = 0
    error_sample_ratio = cfg.sample_mse_portion
    face_sample_ratio = (1 - cfg.sample_mse_portion) * cfg.face_sample_ratio
    body_sample_ratio = (1 - cfg.sample_mse_portion) * cfg.body_sample_ratio
    ray_o_list = []
    ray_d_list = []
    rgb_list = []
    near_list = []
    ...
    while nsampled_rays < nrays:
        n_error = int((nrays - nsampled_rays) * error_sample_ratio)
        n_body = int((nrays - nsampled_rays) * body_sample_ratio)
        n_face = int((nrays - nsampled_rays) * face_sample_ratio)
        n_rand = (nrays - nsampled_rays) - n_error - n_body - n_face

        # sample rays on error part
        coord_error = np.argwhere(error_msk == 1)
        coord_error = coord_error[np.random.randint(0, len(coord_error), n_error)]
        ...
        if len(coord_face) > 0:
            coord = np.concatenate([coord_error, coord_body, coord_face, coord], axis=0)
        else:
            coord = np.concatenate([coord_error, coord_body, coord], axis=0)
...
def get_rays_within_bounds_coord(H, W, K, R, T, bounds):
    ray_o, ray_d, coord = get_rays_coord(H, W, K, R, T)
    ...
    ray_o = ray_o[mask_at_box]
    ray_d = ray_d[mask_at_box]
    coord = coord[mask_at_box]

    mask_at_box = mask_at_box.reshape(H, W)

    return ray_o, ray_d, near, far, mask_at_box, coord
...
def crop_image_msk(img, msk, K, ref_msk):  
    ...
    return crop, crop_msk, K, bbox
    return aligned_image, aligned_msk, K, bbox


def random_crop_image(img, msk, K):
    min_size = max_size = cfg.patch_size
    H, W = img.shape[:2]
    min_HW = min(H, W)
...
def get_grid_points_with_bound(bounds):
    voxel_size = cfg.voxel_size
    x = np.arange(bounds[0, 0], bounds[1, 0] + voxel_size[0], voxel_size[0])
    y = np.arange(bounds[0, 1], bounds[1, 1] + voxel_size[1], voxel_size[1])
    z = np.arange(bounds[0, 2], bounds[1, 2] + voxel_size[2], voxel_size[2])
